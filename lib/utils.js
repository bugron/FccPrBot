...
export function getContent(conf) {
  return new Promise(function(resolve, reject) {
    github.repos.getContent(conf, function(err, body) {
      if (err) {
        reject(err);
      } else {
        resolve(body);
...
export function identifyTranslators(challenge, translationTeams) {
  let result = [], keys = Object.keys(challenge);
  for (let lang in translationTeams) {
    if (
      translationTeams.hasOwnProperty(lang) &&
      keys.indexOf('title' + lang) !== -1 &&
      keys.indexOf('description' + lang) !== -1
    ) {
      result = result.concat(translationTeams[lang]);
    }
  }
  return result;
}
