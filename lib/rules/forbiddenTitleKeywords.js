...
module.exports = async function forbiddenTitleKeywords(repoConfig, data) {
  if (
    repoConfig.rules.forbiddenTitleKeywords &&
    repoConfig.rules.forbiddenTitleKeywords.length
  ) {
    let invalidTitlePattern = '(?:' + repoConfig.rules.forbiddenTitleKeywords.join('|') +
      ')\\s+([\\w\\d-.]*\\/?[\\w\\d-.]*)?#\\d+';
