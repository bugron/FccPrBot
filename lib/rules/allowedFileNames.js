 ...
 */
module.exports = async function allowedFileNamePatterns(repoConfig, data) {
  if (
    repoConfig.rules.critical.allowedFileNamePatterns &&
    repoConfig.rules.critical.allowedFileNamePatterns.length
  ) {
    const githubConfig = {
    ...
    if (filesArr && filesArr.length) {
      filesArr.every((file) => {
        if (file.filename) {
          isValidFileName =
            repoConfig.rules.critical.allowedFileNamePatterns.some((val) => {
